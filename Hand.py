...
class Hand:
    ...
    def __init__(self,cards,tensor=None):
        if tensor is None:
            self.cards=cards
            self.tensor=self.cards_to_tensor()
        else:
            self.tensor=tensor
    ...
    def two_pairs(self):
        ...
        remaining_tensor = self.tensor
        remaining_tensor[:, index_highest, :] = 0
        return -np.sort(-index_highest), self.kickers(remaining_tensor,3)
    ...
    def straight_flush(self):
        ...
        if max_index_flush is not None and self.straight() is not None:
            tensor=self.tensor[:, :, suit_index].reshape(-1,15,4)
            tensor=Hand(tensor)
